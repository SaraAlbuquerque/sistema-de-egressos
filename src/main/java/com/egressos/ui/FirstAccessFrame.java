...
public class FirstAccessFrame extends JFrame {
    ...
    private JPanel buildContent() {
        ...
        c.gridx = 0;
        c.gridy = y;
        form.add(new JLabel("Data de nascimento (AAAA-MM-DD):"), c);
        c.gridx = 1;
    ...
    private void concluirPrimeiroAcesso() {
        ...
        AuthService.ResultadoTrocaObrigatoria r = authService.trocarSenhaObrigatoria(usuario, senhaAtual, novaSenha);
        ...
        Integer anoConc;
        try {
            dataNasc = LocalDate.parse(dataNascimentoField.getText().trim());
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Data de nascimento inválida.");
            return;
        }
        try {
            anoConc = Integer.parseInt(anoConclusaoField.getText().trim());
