...
public class CoordinatorFrame extends JFrame {
    ...
    private JPanel buildUsersTab(){
        ...
        int y=0;
        c.gridx=0;c.gridy=y;filtros.add(new JLabel("Nome contém:"),c);
        c.gridx=1;filtros.add(nomeF,c);
        c.gridx=2;filtros.add(new JLabel("E-mail contém:"),c);
        c.gridx=3;filtros.add(emailF,c);
        y++;
        c.gridx=0;c.gridy=y;filtros.add(new JLabel("Papel:"),c);
        c.gridx=1;filtros.add(papelF,c);
        JButton buscar = new JButton("Atualizar");
        c.gridx=3;filtros.add(buscar,c);
        ...
        Runnable loader = () -> {
            ...
            List<Usuario> filtrado = lista.stream().filter(u -> {
                if (!nf.isEmpty() && (u.getNome()==null || !u.getNome().toLowerCase().contains(nf))) return false;
                if (!ef.isEmpty() && (u.getEmail()==null || !u.getEmail().toLowerCase().contains(ef))) return false;
                if (pf!=null && !pf.isBlank() && !u.getPapel().name().equalsIgnoreCase(pf)) return false;
                return true;
    ...
    private JPanel buildRequestsTab(){
        ...
        remover.addActionListener(e -> {
            ...
            if (row < 0) { JOptionPane.showMessageDialog(this,"Selecione uma solicitação."); return; }
        ...
        aprovar.addActionListener(e -> {
            ...
            Papel papel;
            try { papel = Papel.valueOf(papelStr.toUpperCase()); }
            catch (Exception ex){ JOptionPane.showMessageDialog(this,"Papel inválido: "+papelStr); return; }
    ...
    private interface CsvFn { Path run(Path p) throws Exception; }
